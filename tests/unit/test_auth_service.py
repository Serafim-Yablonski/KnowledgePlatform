...
from src.domain.user import TokenPair
...
class StubUserRepository:
    ...
    async def create(self, data: object, hashed_password: str) -> User:  # type: ignore[override]
        ...
        assert isinstance(data, UserCreationInput)
...
async def test_register_success() -> None:
    ...
    result = await service.register("alice@example.com", "secret99", "Alice")
...
async def test_register_conflict() -> None:
    service, _ = _make_service()
    await service.register("alice@example.com", "secret99")
...
async def test_login_success() -> None:
    service, _ = _make_service()
    await service.register("bob@example.com", "hunter22")
    result = await service.login("bob@example.com", "hunter22")
...
async def test_login_wrong_password() -> None:
    ...
    with pytest.raises(ForbiddenError, match="Invalid credentials"):
        await service.login("bob@example.com", "wrongpass")
...
async def test_login_inactive_user() -> None:
    ...
    user = await service.register("inactive@example.com", "secret99")
    await repo.update(user, is_active=False)
    with pytest.raises(ForbiddenError, match="Invalid credentials"):
        await service.login("inactive@example.com", "secret99")
...
async def test_get_current_user_success() -> None:
    service, _ = _make_service()
    user = await service.register("carol@example.com", "secret99")
...
async def test_get_current_user_inactive() -> None:
    service, repo = _make_service()
    user = await service.register("dave@example.com", "secret99")
    await repo.update(user, is_active=False)
    token = create_access_token(user.id)
    with pytest.raises(ForbiddenError, match="User not found or inactive"):
        await service.get_current_user(token)
...
async def test_refresh_success() -> None:
    ...
    service, _ = _make_service()
    user = await service.register("frank@example.com", "secret99")
...
async def test_refresh_rejects_inactive_user() -> None:
    ...
    service, repo = _make_service()
    user = await service.register("grace@example.com", "secret99")
    await repo.update(user, is_active=False)
    token = create_refresh_token(user.id)
    with pytest.raises(ForbiddenError, match="User not found or inactive"):
        await service.refresh(token)
...
async def test_password_is_hashed_in_repository() -> None:
    ...
    await service.register("eve@example.com", "plaintext1")
