...
async def _register_and_login(client: AsyncClient) -> dict[str, str]:
    await client.post(
        _REGISTER_URL,
        json={"email": _DEFAULT_EMAIL, "password": _DEFAULT_PASSWORD},
    )
...
async def test_register_duplicate_email(async_client: AsyncClient) -> None:
    payload = {"email": _DEFAULT_EMAIL, "password": _DEFAULT_PASSWORD}
...
async def test_register_short_password(async_client: AsyncClient) -> None:
    resp = await async_client.post(
        _REGISTER_URL, json={"email": _DEFAULT_EMAIL, "password": "short"}
    )
...
async def test_login_wrong_password(async_client: AsyncClient) -> None:
    ...
    resp = await async_client.post(
        _LOGIN_URL,
        json={"email": _DEFAULT_EMAIL, "password": "wrongpassword"},
    )
